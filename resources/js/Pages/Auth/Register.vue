            ...
            <div class="mt-4">
                ...
                <div v-if="form.password" class="mt-3 space-y-2">
                    ...
                    <div class="h-1 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                        <div class="h-full transition-all duration-500" :class="passwordStrength.color"
                    ...
                    <ul class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 mt-2">
                        <li v-for="(rule, index) in passwordRules" :key="index"
                            ...
                            <svg v-if="rule.val" xmlns="http://www.w3.org/2000/svg"
                                class="h-3.5 w-3.5 mr-1.5 text-emerald-500 flex-shrink-0" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                            </svg>
                            <span v-else
                                class="w-1 h-1 rounded-full bg-slate-300 dark:bg-slate-700 mr-2.5 ml-1.5 flex-shrink-0"></span>
                            {{ rule.label }}
                        </li>
                    </ul>
                </div>
...
export default {
    ...
    computed: {
        ...
        passwordStrength() {
            ...
            const rulesPassed = this.passwordRules.filter(r => r.val).length;
            if (rulesPassed <= 2) {
